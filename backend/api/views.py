...
from .serializer import UsuarioSerializer, EstudianteSerializer, AsesorSerializer, DocenteSerializer, DirectorSerializer, AjusteSerializer, NotificacionSerializer, AsignaturaSerializer, CasoSerializer, EntrevistaSerializer, InscripcionSerializer, TipoAjusteSerializer, DocumentoSerializer
...
class CrearCasoView(APIView):
    def post(self, request):
        data = request.data
        rut_recibido = data.get('rut_estudiante')

        if not rut_recibido:
            return Response({"error": "El Rut es obligatorio."}, status=status.HTTP_404_NOT_FOUND)

        try:
            estudiante = Estudiante.objects.get(rut=rut_recibido)
